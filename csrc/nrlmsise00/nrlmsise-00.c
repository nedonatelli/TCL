 ...
 *
 * David F. Crouse of the Naval Research Laboratory, Washington, DC
 * modified the function ghp7 to return the computed altitudes, as was the
 * case in the original Fortran code. June 2016. He also modified the code
 * to use mxMalloc and mxFree, which are preferred in Matlab over malloc
 * and free. Also, printf satements have been replaced with the appropriate
...
//DFC: These are not needed when using the functions for Matlab.
 ...
 */
	...
	h = xa[khi] - xa[klo];
	if (h==0.0)
		mexPrintf("bad XA input to splint");
	a = (xa[khi] - x)/h;
	b = (x - xa[klo])/h;
 ...
 */
	...
	double sig, p, qn, un;
	int i, k;
	u=mxMalloc(sizeof(double)*(unsigned int)n);
	if (u==NULL) {
		mexPrintf("Out Of Memory in spline - ERROR");
		return;
	}
	if (yp1>0.99E30) {
		y2[0]=0;
 ...
 */
	...
	if (p[99]==0)
		p[99]=pset;
	if (p[99]!=pset) {
		mexPrintf("Wrong parameter set for glob7s\n");
		return -1;
	}
	for (j=0;j<14;j++)
		t[j]=0.0;
...
double ghp7(struct nrlmsise_input *input, struct nrlmsise_flags *flags, struct nrlmsise_output *output, double press) {
	...
	do {
		...
		if (sqrt(diff*diff)<test)
			return z;
		if (l==ltest) {
			mexPrintf("ERROR: ghp7 not converging for press %e, diff %e",press,diff);
			return z;
		}
