...
def multi_target_tracking_tutorial():
    ...
    for target in targets:
        print(
            f"  Target {target['id']}: start={target['start_pos']}, "
    ...
    for i in range(n_steps):
        ...
        for _ in range(n_false_alarms):
            ...
            false_meas = np.array([np.random.uniform(0, 30), np.random.uniform(0, 20)])
    ...
    for step_idx, (measurements, true_assoc) in enumerate(
        zip(all_measurements, measurement_association)
    ):
