...
def ins_gnss_tutorial():
    ...
    for i in range(1, n_steps):
        ...
        # Integrate position
        ins_position[i] = (
            ins_position[i - 1] + true_velocity[i - 1] * dt + 0.5 * accel_meas * dt**2
        )

        # Simulated heading measurement (with drift)
        heading_rate_meas = (
            angular_velocity + gyro_bias[0] + np.random.randn(1) * gyro_noise
        )
    ...
    # State transition
    F_kf = np.array([[1, 0, dt, 0], [0, 1, 0, dt], [0, 0, 1, 0], [0, 0, 0, 1]])
    ...
    fig.add_trace(
        go.Scatter(
            x=time,
            y=ins_position[:, 0] - true_position[:, 0],
            name="INS Error",
    ...
    fig.add_trace(
        go.Scatter(
            x=time,
            y=fused_position[:, 0] - true_position[:, 0],
            name="Fused Error",
    ...
    fig.add_trace(
        go.Scatter(
            x=time,
            y=ins_position[:, 1] - true_position[:, 1],
            name="INS Error",
            line=dict(color="blue"),
            showlegend=False,
        ),
    ...
    fig.add_trace(
        go.Scatter(
            x=time,
            y=fused_position[:, 1] - true_position[:, 1],
            name="Fused Error",
            line=dict(color="green"),
            showlegend=False,
        ),
