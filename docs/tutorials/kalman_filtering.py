...
def kalman_filtering_tutorial():
    ...
    print(f"True trajectory range: x=[{true_states[:, 0].min():.2f}, {true_states[:, 0].max():.2f}], "
    ...
    print(f"Filter complete. RMSE: {np.sqrt(np.mean((estimates - true_states)**2)):.4f}")
    ...
    fig.add_trace(
        go.Scatter(x=time, y=true_states[:, 0], name="True X", 
                   line=dict(color="black", dash="dash"), showlegend=False),
        row=1, col=2
    )
    fig.add_trace(
        go.Scatter(x=time, y=measurements[:, 0], name="Measured X",
                   mode="markers", marker=dict(color="red", size=3), showlegend=False),
        row=1, col=2
    )
    fig.add_trace(
        go.Scatter(x=time, y=estimates[:, 0], name="Estimated X",
    ...
    fig.add_trace(
        go.Scatter(x=time, y=true_states[:, 1], name="True Y",
                   line=dict(color="black", dash="dash"), showlegend=False),
        row=2, col=1
    )
    fig.add_trace(
        go.Scatter(x=time, y=measurements[:, 1], name="Measured Y",
                   mode="markers", marker=dict(color="red", size=3), showlegend=False),
        row=2, col=1
    )
    fig.add_trace(
        go.Scatter(x=time, y=estimates[:, 1], name="Estimated Y",
                   line=dict(color="blue"), showlegend=False),
        row=2, col=1
    )
