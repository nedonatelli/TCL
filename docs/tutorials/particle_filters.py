...
def particle_filters_tutorial():
    ...
    for k in range(n_steps):
        ...
        if 1.0 / np.sum(weights**2) < n_particles / 2:  # ESS criterion
            indices = np.argsort(np.random.rand(n_particles) - np.cumsum(weights))[:n_particles]
    ...
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=x_true[:, 0],
                   name="True", mode="lines", line=dict(color="black", width=3, dash="dash")),
        row=1, col=1
    )
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=x_est_bpf[:, 0],
                   name="Particle Filter", mode="lines", line=dict(color="blue", width=2)),
        row=1, col=1
    )
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=x_ekf[:, 0],
                   name="Extended KF", mode="lines", line=dict(color="red", width=2, dash="dot")),
        row=1, col=1
    )
    ...
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=x_est_bpf[:, 1],
                   name="Particle Filter", mode="lines", line=dict(color="blue", width=2),
                   showlegend=False),
        row=1, col=2
    )
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=x_ekf[:, 1],
                   name="Extended KF", mode="lines", line=dict(color="red", width=2, dash="dot"),
                   showlegend=False),
        row=1, col=2
    )
    ...
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=error_bpf,
                   name="BPF Error", mode="lines", line=dict(color="blue", width=2)),
        row=2, col=1
    )
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=error_ekf,
                   name="EKF Error", mode="lines", line=dict(color="red", width=2)),
        row=2, col=1
    )
    ...
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=z_all.flatten(),
                   name="Measurements", mode="markers", marker=dict(color="gray", size=4, opacity=0.6)),
        row=2, col=2
    )
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=x_est_bpf[:, 0]**2,
                   name="BPF Prediction", mode="lines", line=dict(color="blue", width=2)),
        row=2, col=2
    )
    fig.add_trace(
        go.Scatter(x=np.arange(n_steps), y=x_ekf[:, 0]**2,
                   name="EKF Prediction", mode="lines", line=dict(color="red", width=2, dash="dot")),
        row=2, col=2
    )
