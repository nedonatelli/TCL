...
def data_association_tutorial():
    ...
    for k in range(n_steps):
        ...
        meas_list.append(
            np.array(measurements) if measurements else np.array([]).reshape(0, 2)
        )
        ...
        if n_tracks > 0 and n_meas > 0:
            ...
            for j in range(n_meas):
                if j not in used_meas:
                    tracks_gnn.append(
                        [measurements[j][0], measurements[j][1], 0, 0, 0, 0]
                    )
    ...
    for k in range(n_steps):
        ...
        if n_tracks > 0 and n_meas > 0:
            ...
            # Create new tracks
            used_meas = set(
                meas_indices[np.where(cost_matrix[track_indices, meas_indices] < 1000)]
            )
            for j in range(n_meas):
                if j not in used_meas:
                    tracks_hungarian.append(
                        [measurements[j][0], measurements[j][1], 0, 0, 0, 0]
                    )
    ...
    fig.add_trace(
        go.Scatter(
            x=np.arange(n_steps),
            y=measurements_assoc_hungarian,
            name="Hungarian Tracks",
            mode="lines",
