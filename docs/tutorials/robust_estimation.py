...
def robust_estimation_tutorial():
    ...
    for iteration in range(10):
        ...
        c = 1.0  # threshold
        weights = np.where(
            np.abs(residuals) <= c, np.ones(n_data), c / np.abs(residuals)
        )
    ...
    fig.add_trace(
        go.Scatter(
            x=x_data[best_inlier_mask],
            y=y_data[best_inlier_mask],
            mode="markers",
            name="Inliers",
