...
def signal_processing_tutorial():
    ...
    print(f"Signal components: 50 Hz (1.0), 150 Hz (0.5), 200 Hz (0.3), Gaussian noise (0.2)")
    ...
    print(f"Spectrogram computed: {f.shape[0]} frequency bins, {Sxx.shape[1]} time bins")
    
    # Compute spectrogram of filtered signal
    f_filt, t_spec_filt, Sxx_filt = spectrogram(filtered_signal, fs, nperseg=256, noverlap=200)
    ...
    for freq, name in [(50, "50 Hz"), (150, "150 Hz"), (200, "200 Hz")]:
        fig.add_vline(x=freq, line_dash="dash", line_color="gray", row=1, col=2,
    ...
    fig.add_trace(
        go.Scatter(
            x=freqs_pos, y=magnitude_filtered_pos,
    ...
    for freq, name in [(50, "50 Hz"), (150, "150 Hz"), (200, "200 Hz")]:
        fig.add_vline(x=freq, line_dash="dash", line_color="gray", row=2, col=2,
                      annotation_text="", annotation_position="top")
    
    # Row 3: Spectrograms
    fig.add_trace(
        go.Heatmap(
            x=t_spec, y=f, z=10*np.log10(Sxx + 1e-10),
            colorscale="Viridis",
            showscale=False,
            name="Original",
        ),
    ...
    fig.add_trace(
        go.Heatmap(
            x=t_spec_filt, y=f_filt, z=10*np.log10(Sxx_filt + 1e-10),
            colorscale="Viridis",
            showscale=True,
            name="Filtered",
        ),
