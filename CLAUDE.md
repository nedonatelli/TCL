...
`astronomy`, `geodesy`, `terrain` (netCDF4), `storage` (h5py), `visualization` (plotly), `optimization` (cvxpy), `signal` (pywavelets), `gpu` (cupy), `gpu-apple` (mlx), `dev` (test/lint/docs tooling), `all` (everything except gpu)
