...
Requires a DE440 kernel cached at ``~/.jplephem/de440.bsp``; skips (never
downloads) when it is absent.
"""
...
requires_kernel = pytest.mark.skipif(
    not os.path.exists(_kernel_path()),
    reason="DE440 kernel not cached at ~/.jplephem/de440.bsp",
)
...
def test_moon_earth_centered_is_geocentric_not_barycentric():
    ...
    assert got_km == pytest.approx(expected_km, rel=1e-9)
...
def test_planet_position_earth_is_earth_not_the_barycentre():
    ...
    assert np.allclose(earth_km, expected_km, rtol=1e-9)
