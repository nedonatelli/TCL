...
def demo_bessel_functions() -> None:
    ...
    # First kind
    j0 = np.array([bessel_j0(xi) for xi in x])
    j1 = np.array([bessel_j1(xi) for xi in x])

    # Second kind
    y0 = np.array([bessel_y0(xi) for xi in x])
    y1 = np.array([bessel_y1(xi) for xi in x])

    print(f"\nBessel Functions J_n and Y_n:")
    print(f"  J_0(1) = {bessel_j0(1.0):.6f}")
    print(f"  J_1(1) = {bessel_j1(1.0):.6f}")
    print(f"  Y_0(1) = {bessel_y0(1.0):.6f}")
    print(f"  Y_1(1) = {bessel_y1(1.0):.6f}")
...
def demo_higher_order_bessel() -> None:
    ...
    print(f"\nBessel functions at x={x_val}:")
    for n in range(5):
        j_n = bessel_jn(n, x_val)
        y_n = bessel_yn(n, x_val)
        print(f"  J_{n}({x_val}) = {j_n:.6f}, Y_{n}({x_val}) = {y_n:.6f}")
    ...
    if SHOW_PLOTS:
        ...
        for n in range(5):
            jn_vals = np.array([bessel_jn(n, xi) for xi in x])
            fig.add_trace(
                go.Scatter(
                    x=x,
                    y=jn_vals,
