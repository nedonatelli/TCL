    ...
    if dtype is not None:
        try:
            ...
            raise ValidationError(
                f"Cannot convert {name} to dtype {dtype}: {e}"
            ) from e
...
def ensure_column_vector(arr: ArrayLike, name: str = "vector") -> NDArray[Any]:
    ...
    if result.ndim == 1:
        ...
        raise ValidationError(
            f"{name} must be 1D or 2D, got {result.ndim}D"
        )
...
def ensure_square_matrix(arr: ArrayLike, name: str = "matrix") -> NDArray[Any]:
    ...
    if result.shape[0] != result.shape[1]:
        raise ValidationError(
            f"{name} must be square, got shape {result.shape}"
        )
...
def ensure_positive_definite(
    ...
    try:
        ...
        raise ValidationError(
            f"Could not compute eigenvalues of {name}: {e}"
        ) from e
