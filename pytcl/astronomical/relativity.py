...
def shapiro_delay(observer_pos: np.ndarray, light_source_pos: np.ndarray,
                  gravitating_body_pos: np.ndarray, gm: float = GM_SUN) -> float:
...
def post_newtonian_acceleration(r_vec: np.ndarray, v_vec: np.ndarray,
                                 gm: float = GM_EARTH) -> np.ndarray:
...
def geodetic_precession(a: float, e: float, inclination: float,
...
def lense_thirring_precession(a: float, e: float, inclination: float,
...
def relativistic_range_correction(distance: float, relative_velocity: float,
                                   gm: float = GM_EARTH) -> float:
